  ...
  try {
    ...
    if (user.credits < 50) {
      return res.status(400).json({
        message:
          "Not enough credits.Minimum 50 credits required to start interview",
      });
    }
    const projectText =
    ...
    if (!userPrompt.trim()) {
      return res
        .status(400)
        .json({ message: "Insufficient data to generate questions" });
    }
    const messages = [
      {
        role: "system",
...
export const getInterviewReport = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: `Failed to get interview report: ${error.message}` });
  }
};
